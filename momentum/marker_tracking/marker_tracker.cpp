...
std::vector<size_t> sampleFrames(
  ...
  const size_t numFrames = static_cast<size_t>(initialMotion.cols());
  ...
  // // find the frames with the min/max for each parameters
  // const size_t numParams = normalized.rows();
  // std::unordered_set<size_t> frameSet;
  // for (size_t i = 0; i < numParams; ++i) {
  //   size_t minFrame;
  //   size_t maxFrame;
  //   normalized.row(i).minCoeff(&minFrame, &maxFrame);
  //   frameSet.insert(minFrame);
  //   frameSet.insert(maxFrame);
  // }

  // // insert the frame indices from the frame set into the result
  // frameIndices.insert(frameIndices.end(), frameSet.begin(), frameSet.end());
  ...
  //   for (size_t fi = 0; fi < solvedFrames; fi++) {
  //     const float dist =
  //         (normalized.col(frameIndices[i]) - normalized.col(fi)).cwiseAbs().maxCoeff();
...
float getLocatorError(
  ...
  for (size_t iFrame = 0; iFrame < numFrames; ++iFrame) {
    ...
    for (const auto& jMarker : markerList) {
      ...
      if (locator.name == "ARIEL" || locator.name.find("HD") != std::string::npos) {
        frameHeadError += markerError;
        maxHeadError = std::max(maxHeadError, static_cast<double>(markerError));
        validHeadMarkers++;
      }
    }
    ...
    if (validHeadMarkers > 0) {
      headError += frameHeadError / validHeadMarkers;
    }
  }
  MT_LOGI("Average marker error: {}cm", error / numFrames);
  MT_LOGI("Max marker error: {}cm", maxError);
  MT_LOGI("Average head marker error: {}cm", headError / numFrames);
  MT_LOGI("Max head marker error: {}cm", maxHeadError);

  return error / numFrames;
}
